  ...
  val vectorCodecSpec =
    suite("ArrowVectorCodec")(
      test("decoder map") {
        ZIO.scoped(
          for {
            intVec <- VectorEncoder[Int, IntVector].encodeZIO(Chunk(1, 2, 3))
            result <- VectorDecoder[IntVector, Int].map(_.toString).decodeZIO(intVec)
          } yield assert(result)(equalTo(Chunk("1", "2", "3")))
        )
      },
      test("decoder flatMap") {
        val decoder = VectorDecoder[IntVector, Int]
      ...
      test("codec empty") {
        ZIO.scoped(
          for {
            scalarVec    <- VectorEncoder[Int, IntVector].encodeZIO(Chunk.empty)
            scalarResult <- VectorDecoder[IntVector, Int].decodeZIO(scalarVec)
            listVec      <- VectorEncoder[List[Int], ListVector].encodeZIO(Chunk.empty)
            listResult   <- VectorDecoder[ListVector, List[Int]].decodeZIO(listVec)
      ...
      test("codec boolean") {
        ZIO.scoped(
          for {
            vec    <- VectorEncoder[Boolean, BitVector].encodeZIO(Chunk(true, true, false))
      ...
      test("codec long") {
        ZIO.scoped(
          for {
            vec    <- VectorEncoder[Long, BigIntVector].encodeZIO(Chunk(1L, 2L, 3L))
      ...
      test("codec string") {
        ZIO.scoped(
          for {
            vec    <- VectorEncoder[String, VarCharVector].encodeZIO(Chunk("zio", "cats", "monix"))
      ...
      test("codec list boolean") {
        ZIO.scoped(
          for {
            vec    <- VectorEncoder[Set[Boolean], ListVector].encodeZIO(Chunk(Set(true), Set(false, true)))
            result <- VectorDecoder[ListVector, List[Boolean]].decodeZIO(vec)
      ...
      test("codec list long") {
        ZIO.scoped(
          for {
            vec    <- VectorEncoder[Set[Long], ListVector].encodeZIO(Chunk(Set(1L, 2L), Set(3L)))
            result <- VectorDecoder[ListVector, List[Long]].decodeZIO(vec)
