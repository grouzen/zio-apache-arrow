...
object ValueVectorEncoder {
  ...
  //           it.foreach { case (v, i) =>
  //             encodePrimitive(standardType, v, vec, i)
  //           }

  //           vec.setValueCount(len)
  //           vec
  //         case other                             =>
  //           throw EncoderError(s"Unsupported ZIO Schema type $other, must be of type Schema.Primitive[A]")
  ...
  //             encodeStruct(v, record.fields, writer)
